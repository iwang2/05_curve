...
The file follows the following format:
	 ...
	 display: draw the lines of the edge matrix to the screen
	    display the screen
	 draw: display without the displaying
	 reset: clear edge matrix
	 save: draw the lines of the edge matrix to the screen
	    save the screen to a file -
	    takes 1 argument (file name)
	 color: 3 args (red, green, blue)
	 step: change step
	 quit: end parsing
...
void parse_file ( char * filename, 
  ...
  while ( fgets(line, sizeof(line), f) != NULL ) {
    ...
    if ( strncmp(line, "color", strlen(line)) == 0 ) {
      ...
      sscanf(line, "%d %d %d", &c.red, &c.green, &c.blue);
    }

    else if ( strncmp(line, "step", strlen(line)) == 0 ) {
      fgets(line, sizeof(line), f);
      sscanf(line, "%d", &step);
    }
      ...
      add_curve(edges, xvals[0], yvals[0], xvals[1], yvals[1],
		rx[0], ry[0], rx[1], ry[1], step, HERMITE);
    }
    ...
    else if ( strncmp(line, "reset", strlen(line)) == 0 ) {
      edges->lastcol = 0;
    }
